...
func _draw_mud_patch(center: Vector2, radius: float, strength: float, seed_value: float) -> void:
	# 세척 진행에 따라 진한 흙색에서 밝은 모래색으로 색이 변해 청소 진행감을 직관적으로 전달한다.
	var base_color := Color(
		lerp(0.72, 0.36, strength),
		lerp(0.58, 0.19, strength),
		lerp(0.44, 0.08, strength),
		0.82 * strength
	)
	draw_circle(center, radius * (0.8 + strength * 0.25), base_color)
	for index in range(5):
