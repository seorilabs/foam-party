...
//
// The game registers a back handler via window.__foamPartyNav.setBackHandler and
// calls window.__foamPartyNav.closeApp() to quit. If no handler is registered yet
// (game still booting), a back press closes the view — the framework default.

type FoamPartyNavBridge = {
...
declare global {
  interface Window {
    __foamPartyNav?: FoamPartyNavBridge
  }
}
...
export function installFoamPartyGraniteNavBridge() {
  let backHandler: (() => void) | null = null

  window.__foamPartyNav = {
