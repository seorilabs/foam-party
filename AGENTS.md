...
- Godot compile, Godot Web build, docs/core/architecture checks는 repo가 private이면 `seorilabs-x64`, public이면 `ubuntu-latest`로 간다. workflow의 `github.event.repository.private` 가드가 이 분기를 담당한다.
